...
@foreach ($related as $key => $rel)
    ...
    <div class="col-md-12 block1">
        @if (isset($type) && $type == 'grocery')
            ...
            <div class="content1"> 
                <a href="../{{config('app.defaultBaseURL.grocery-store-details')}}/{{ $rel['urlName'] }}" title="{{$rel['name']}}" class="title"><h1 class="colorh1">{{ $rel['name'] }}</h1></a>
            </div>
        ...
        @if(isset($type) && $type == 'restaurant')
            ...
            <div class="content1"> 
                <a href="../{{config('app.defaultBaseURL.dallas-indian-restaurant')}}/{{ $rel['urlName'] }}" title="{{$rel['name']}}" class="title"><h1 class="colorh1">{{ $rel['name'] }}<h1></a>
            </div>
            ...
            <div class="smallImage">
                ...
                <div class="content1"> 
                    @if ($rel['religionName'] == 'Christianity')
                        <a href="{{ URL::to('/') }}/{{config('app.defaultBaseURL.dallas-christian-church')}}/{{ $rel['urlName'] }}" title="{{$rel['name']}}" class="title"><h1 class="colorh1">{{ $rel['name'] }}<h1></a>
                    @elseif($rel['religionName'] == 'Hinduism')
                        <a href="{{ URL::to('/') }}/{{config('app.defaultBaseURL.dallas-hindu-temple')}}/{{ $rel['urlName'] }}" title="{{$rel['name']}}" class="title"><h1 class="colorh1">{{ $rel['name'] }}<h1></a>
                    ...
                    @elseif($rel['religionName'] == 'Islam')
                        <a href="{{ URL::to('/') }}/{{config('app.defaultBaseURL.dallas-islan-mosque')}}/{{ $rel['urlName'] }}" title="{{$rel['name']}}" class="title"><h1 class="colorh1">{{ $rel['name'] }}<h1></a>                                                        
                    @endif
                </div>
                <a href="https://www.google.com/maps/dir/{{$rel['latitude']}},{{$rel['longitude']}}" title="{{$rel['name']}}" target="_blank" class="mapicon"><img src="{{ URL::to('/') }}/image/map1.svg" alt="{{$loop->index}}{{ $rel['name'] }}"/></a>
                <div class="content2"> <span class="subcontent2">{{ $rel['address1'] }} {{ $rel['address2'] }}, {{ $rel['city'] }}, {{ $rel['state'] }} {{ $rel['zip'] }}</span> </div>
