              ...
              <div class="panel panel-default">    
                  ...
                  <div class="panel-body">
                        <ul class="nav nav-tabs">
                            ...
                            <li><a href="#pending" data-toggle="tab">Pending ({{count($movies_pending)}})</a> </li>                        
                        </ul>  
                            ...
                            <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-pending">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Language</th>
                                        <th data-sortable="false" width="100px;">Link</th>
                                        <th data-sortable="false" width="150px;">Action</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($movies_pending as $key => $rel)
                                        @if ($rel['status'] ==4)
                                            <tr class="odd gradeX danger">
                                        @else
                                            <tr class="odd gradeX">
                                        @endif    
                                                ...
                                                <td>{{config('app.movieLanguage.'.$rel["language"])}}</td>
                                                <td class="ceter" width="100px;">
                                                    <a href="{{ URL::to('/') }}/review/movie/{{ $rel['urlName'] }}" target="_blank" class="title">Link</a>    
                                                </td>
                                                <td width="150px;">
                                                    <button type="button" class="btn btn-default btn-sm" onClick="deleteTempMovie({{$rel['movieId']}})"><i class="fa fa-trash-o"></i></button>
                                                    <button type="button" class="btn btn-default btn-sm" onClick="approveMovie({{$rel['movieId']}})"><i class="fa fa-thumbs-up"></i></button>
                                                    <button data-toggle="modal" data-target="#myModal" data-val="{{$rel['movieId']}}"  data-whatever="@mdo" class="btn btn-default btn-sm"><i class="fa fa-thumbs-down"></i></button>                                                    
                                            </td>
                                            </tr>
                                    @endforeach
                                </tbody>
...
<div class="loading-overlay">
  ...
  <script>
    ...
    function deleteTempGrocery(id) {
        var txt;
        var r = confirm("Are you sure to delete");
        if (r == true) {
            window.location.href = "/admin/movie_tmp/delete/"+id;
        }
    }  
    function approveMovie(id){
        var r = confirm("Are you sure to approve");
        if (r == true) {
            window.location.href = "/admin/movie/approve/"+id;
        }
    }              
    </script> 
@endsection
